...
@section('title', 'Car Care - Payment')
@section('content')
<main class="bg-[#FAFAFA] max-w-[640px] mx-auto min-h-screen relative flex flex-col has-[#CTA-nav]:pb-[120px] has-[#Bottom-nav]:pb-[120px]">
      ...
      <div id="Location" class="flex flex-col gap-2">
        ...
        <div class="flex items-center w-full gap-[10px] bg-white">
          <div class="w-[80px] h-[60px] flex shrink-0 rounded-xl overflow-hidden">
            <img src="{{ asset('assets/images/thumbnails/th-details-1.png') }}" class="w-full h-full object-cover" alt="thumbnail">
          </div>
          <div class="flex flex-col">
            <div class="flex items-center gap-1">
              <p class="font-semibold w-fit">{{ $carStore->name }}</p>
              <div class="w-[18px] h-[18px] flex shrink-0">
                <img src="{{ Storage::url($carStore->thumbnail) }}" alt="verified">
              </div>
            </div>
  ...
  <div class="flex h-full flex-1 mt-5">
    <form method="POST" action="{{ route('front.booking.payment.store') }}" enctype="multipart/form-data" 
